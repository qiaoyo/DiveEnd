...
- **DeepStart**：自然语言输入研究方向，对每个原始/改写 query 并行检索 Semantic Scholar、OpenAlex、arXiv、OpenReview 和 DBLP，再按 DOI、arXiv ID、OpenReview forum 和规范化题名合并版本，使用统一的短语/词项覆盖、元数据质量、来源一致性、引用与时间信号过滤排序。所有主题使用同一套来源和规则，默认收敛到约 20 篇高相关候选；只为排名最前的 4 篇预取和结构抽取，其余候选按需处理。
...
- Semantic Scholar is enabled with an optional local key. Without a key it uses a shared anonymous request gate at 0.5 RPS and degrades immediately on shared-pool `429`; with a key it uses a 1 RPS gate and bounded retries. All five sources degrade independently.
...
- Opt-in retrieval E2E (`DIVEEND_REAL_SEARCH_E2E=1`) verifies all rewritten queries, five-source partial-failure behavior, version merging, and top-five topic relevance. The 2026-07-31 no-key run completed in 29.7 seconds despite Semantic Scholar anonymous-pool `429` responses and intermittent DBLP failures; it still placed five code/LLM/agent papers in the top five.
...
- The configured OpenAlex key passed a real skill rate-limit probe and product retrieval E2E. Semantic Scholar remains usable without a key at a conservative application-controlled rate; a replacement key can be added later without changing the retrieval pipeline.
