...
- The OpenAlex key passed a real rate-limit probe and multi-query product E2E on 2026-07-31. A five-source no-key E2E then passed in 29.7 seconds: the current Semantic Scholar anonymous pool returned `429`, degraded immediately for each query, and did not prevent relevant four-source results. Semantic Scholar remains enabled and its local key may remain empty.
